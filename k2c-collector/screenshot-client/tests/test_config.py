...
def test_settings_with_required_env(monkeypatch):
    ...
    monkeypatch.setenv("API_ENDPOINT", "http://localhost:8000/upload")

    settings = Settings(_env_file=None)

    assert settings.api_endpoint == "http://localhost:8000/upload"
...
def test_settings_with_custom_values(monkeypatch):
    ...
    monkeypatch.setenv("API_ENDPOINT", "http://api.example.com/screenshots")
    ...
    assert settings.api_endpoint == "http://api.example.com/screenshots"
